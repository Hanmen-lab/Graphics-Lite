    ...
    internal static class SEGIInspector
    {
        ...
        internal static void Draw(LightManager lightManager, GlobalSettings renderSettings)
        {
            ...
            if (SEGIManager.settings != null)
            {
                ...
                //    }
                //    GUILayout.Space(35);
                //    Label("SEGI - Available in Deferred Rendering Mode Only", "", false);
                //}
                //else
                //{
                    if (Graphics.Instance.CameraSettings.RenderingPath != CameraSettings.AIRenderingPath.Deferred)
                    ...
                    if (segiSettings.enabled)
                    {
                        ...
                        GUILayout.BeginVertical(EmptyBox);
                        {
                            ...
                            GUILayout.Space(5);
                            Slider("Sky Intensity", segiSettings.skyIntensity, 0f, 8f, "N2", intensity => { segiSettings.skyIntensity = intensity; SEGIManager.UpdateSettings(); });
                            ...
                            GUILayout.Space(1);
                            Label("Require Deferred Rendering mode and Deferred Shaders. Shaders that not support Deffered will be semi transparent/bugged.", "", false);
                            GUILayout.Space(1);
                            LabelColorRed("Not Supported: AIT/Clothes True, AIT/Item, AIT/Skin True Face, ShaderForge/hair08, all Alpha Shaders... etc.", "", false);
                            Label("For correct setup use:", "", false);
                            Label("Character: Next-Gen Body, Next Gen Face + Turn off SSS checkbox.", "", false);
                            Label("Clothing: Hanmen/Clothes True Cutoff", "", false);
                            Label("Accessories: Hanmen/Item Cutoff", "", false);
                            Label("Studio Items (if bugged): Hanmen/Item Cutoff or Standard.", "", false);
                        }
