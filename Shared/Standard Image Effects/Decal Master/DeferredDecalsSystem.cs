    ...
    public class DeferredDecalsSystem : MonoBehaviour
    {
        ...
        public static Material DefaultDecalMaterial
        {
            get
            {
                ...
                {
                    AssetBundle assetBundle = AssetBundle.LoadFromMemory(ResourceUtils.GetEmbeddedResource("decals-resources.unity3d"));
                    defaultDecalMaterial = assetBundle.LoadAsset<Material>("Assets/Knife/Decal Master/Resources/Knife/Deferred Decals/DefaultDecalMaterial.mat");
                }
        ...
        public void Enable()
        {  
            assetBundle = AssetBundle.LoadFromMemory(ResourceUtils.GetEmbeddedResource("decals-resources.unity3d"));
            //specularSmoothnessBlitterShader = Shader.Find("Hidden/Knife/SpecSmoothnessBlitter");
            specularSmoothnessBlitterShader = assetBundle.LoadAsset<Shader>("Assets/Knife/Decal Master/Resources/Knife/Deferred Decals/SpecSmoothnessBlitter.shader");
            ...
            depthShader = assetBundle.LoadAsset<Shader>("Assets/Knife/Decal Master/Resources/Knife/Deferred Decals/DepthToTarget.shader");
        ...
        private void CreateCubeMeshCopy()
        {
            ...
            if (cubeMesh == null)
            {
                Debug.LogError("Could not load built-in cube mesh. Halting operation.");
                return;
            }

            cubeMeshCopy = Instantiate(cubeMesh);
            cubeMeshCopy.name = "Pivoted Cube Mesh";
            ...
            cubeMeshCopy.RecalculateBounds();
            cubeMeshCopy.RecalculateNormals();
        ...
        public void Render(Camera camera)
        {
            ...
            foreach (var decal in sortedDecalsBySortingOrder)
            {
                ...
                isCamInsideDecal = IsPointInsideDecal(decal.transform, cameraPosition);
                ...
                groupKey.CameraInside = isCamInsideDecal;
            ...
            foreach (var decalGroup in decalsGrouped)
            {
                ...
                int passOffset = decalGroup.Key.CameraInside ? 3 : 0; // pass offset used to render decal if camera inside in decal
                if (decalGroup.Key.Instancing)
                {
                    ...
                    for (int i = 0; i < batchCount; i++)
                    {
                        ...
                        if (drawCount > 0)
                        {
                            ...
                            bufferBeforeLighting.DrawMeshInstanced(CubeMesh, 0, decalGroup.Key.Material, 0 + passOffset, matrices, drawCount, instancedBlock);
                        }
                    }
                }
                else
                {
                    foreach (var decal in decalsList)
                    {
                        ...
                        bufferBeforeLighting.DrawMesh(CubeMesh, decal.transform.localToWorldMatrix, decal.DecalMaterial, 0, 0 + passOffset);
                    }
                }
            }
            ...
            foreach (var decalGroup in decalsGrouped)
            {
                ...
                if (decalGroup.Key.Instancing)
                {
                    ...
                    for (int i = 0; i < batchCount; i++)
                    {
                        ...
                        if (drawCount > 0)
                        {
                            instancedBlock.SetVectorArray("_Tint", tintArray);
                            instancedBlock.SetVectorArray("_UV", uvArray);
                            bufferBeforeLighting.DrawMeshInstanced(CubeMesh, 0, decalGroup.Key.Material, 2 + passOffset, matrices, drawCount, instancedBlock);
                        }
                    }
                }
                else
                {
                    foreach (var decal in decalsList)
                    {
                        ...
                        uv = decal.UV;
                        bufferBeforeLighting.SetGlobalVector("_NotInstancedUV", uv);
                        bufferBeforeLighting.DrawMesh(CubeMesh, decal.transform.localToWorldMatrix, decal.DecalMaterial, 0, 2 + passOffset);
                    }
                }
            }
            ...
            if (UnityEngine.Terrain.activeTerrains == null || Terrain.activeTerrains.Length == 0)
            {
                foreach (var d in system.m_Decals)
                {
                        ...
                        if (go != null)
                        {
                            ...
                            if (decal != null)
                            {
                                ...
                                if (lastSelectedDecals.TryGetValue(decal, out time))
                                {
                                    ...
                                    if (isCamInsideDecal)
                                    {
                                        selectionBuffer.DrawMesh(CubeMesh, decal.transform.localToWorldMatrix, decal.DecalMaterial, 0, 1 + 3); // selection pass - 1 + 3 (front + greater)
                                    }
                                    else
                                    {
        ...
        public void CopyHeightmaps()
        {
            ...
            UnityEngine.Graphics.ExecuteCommandBuffer(copyHeightmapsBuffer);
        ...
        public bool IsPointInsideDecal(Transform decalTransform, Vector3 point)
        {
            var localPoint = decalTransform.InverseTransformPoint(point);

            var inside = localPoint.x > -0.5f && localPoint.x < 0.5f
            && localPoint.y > -1f && localPoint.y < 0f
