...
namespace Graphics.AmplifyOcclusion
{
	...
	private bool m_prevIsPlaying = false;
        ...
        static private AssetBundle assetBundle;
        static private Shader occ_shader;
        static private Shader blur_shader;
        static private Shader apply_shader;

        void Awake()
        {
            //Load shaders from Assetbundle
            assetBundle = AssetBundle.LoadFromMemory(ResourceUtils.GetEmbeddedResource("amplifyocc.unity3d"));
            if (assetBundle == null) Graphics.Instance.Log.Log(BepInEx.Logging.LogLevel.Error, "failed to load asset bunle 'amplifyocc.unity3d'");
            ...
            DontDestroyOnLoad(apply_shader);
        ...
        private void checkMaterials()
        {
            ...
            {
                m_occlusionMat = new Material(occ_shader);
            }

            if (m_blurMat == null)
            {
                m_blurMat = new Material(blur_shader);
            }

            if (m_applyOcclusionMat == null)
            {
                m_applyOcclusionMat = new Material(apply_shader);
            }
            ...
            if (m_targetCamera != null)
            {
                if (m_targetCamera.actualRenderingPath != RenderingPath.DeferredShading)
                {
                    if (PerPixelNormals != PerPixelNormalSource.None && PerPixelNormals != PerPixelNormalSource.Camera)
                    {
                        ...
                        AmplifyOccManager.settings.PerPixelNormals = (Settings.AmplifyOccSettings.PerPixelNormalSource)PerPixelNormalSource.Camera;
                    ...
                    if (ApplyMethod == ApplicationMethod.Deferred)
                    {
                        ...
                        AmplifyOccManager.settings.ApplyMethod = (Settings.AmplifyOccSettings.ApplicationMethod)ApplicationMethod.PostEffect;
                        AmplifyOccManager.UpdateSettings();
                ...
                else
                {
                    if (PerPixelNormals == PerPixelNormalSource.Camera)
                    {
                        ...
                        AmplifyOccManager.settings.PerPixelNormals = (Settings.AmplifyOccSettings.PerPixelNormalSource)PerPixelNormalSource.GBuffer;
                        AmplifyOccManager.UpdateSettings();
        ...
        private void commandBuffer_FillComputeOcclusion(CommandBuffer cb)
        {
            ...
            if ((PerPixelNormals == PerPixelNormalSource.GBuffer) ||
                (PerPixelNormals == PerPixelNormalSource.OctaEncoded))
            {
